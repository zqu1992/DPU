...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='dpu', help='The partial updating mode, allowed: dpu, gcpu, lcpu')
    ...
    parser.add_argument('--without_reinit', action='store_true', help='Do not use re-initialization, default: use re-initialization')
    ...
    for r in range(num_rounds):
        ...
        if r == 0:
            ...
            if (len(train_dataset) > 2*num_samples) and (not args.without_reinit):
        ...
        for epoch in range(args.num_epochs):
            ...
            train(model, train_loader, loss_func, optimizer_w, optimizer_b, DPU_layers, epoch, first_step=True)
        ...
        # Compute the rewinding metric according to the chosen partial updating mode
        if args.mode.lower() == 'dpu':
            for p_dpu in DPU_layers:
                p_dpu.metric.add_(args.lamda*p_dpu.global_contribution+(1-args.lamda)*p_dpu.local_contribution)
        elif args.mode.lower() == 'gcpu':
            for p_dpu in DPU_layers:
                p_dpu.metric.add_(p_dpu.global_contribution)
        elif args.mode.lower() == 'lcpu':
            for p_dpu in DPU_layers:
        ...
        act_idx = sorted_idx[:int(args.updating_ratio*num_weights_total)]
        ...
        # Record the validation and test accuracy at the start point of the second step in round r
        ...
        # Check if the validation accuracy after rewinding is higher than the start point to avoid unnecessary updating
