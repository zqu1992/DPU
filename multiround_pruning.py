...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='pruning', help='The mode (just for the saved model name), allowed: pruning')
    ...
    parser.add_argument('--pruning_ratio', type=float, required=True, help='The pruning ratio, i.e., the ratio of nonzero weights')
    ...
    print('The pruning ratio: ', args.pruning_ratio)
    ...
    if args.model.lower() == 'mlp':
        ...
        args.num_epochs = args.num_epochs if args.num_epochs else 30
    ...
    for r in range(num_rounds):
        ...
        # Load the the saved initialized random model
        ...
        # DPU module is used for pruning
        ...
        print('Pruning...')
        # Update the metric by the absolute value of weights
        ...
        for i, (p_dpu, p) in enumerate(zip(DPU_layers, parameters_w)):
            ...
            i_metric_list = p_dpu.sort_metric(int(p_dpu.w_init.nelement()*0.65)) 
        ...
        act_idx = sorted_idx[:int(args.pruning_ratio*num_weights_total)]
        act_list = metric_list[act_idx,:]
        # Prune the weights and generate the start point of sparse fine-tuning
        for i, (p_dpu, p) in enumerate(zip(DPU_layers, parameters_w)):
            p_dpu.update_mask(act_list[act_list[:,0]==i,1].long())
            p.data.mul_(p_dpu.mask)

        # Record the validation and test accuracy at the start point of sparse fine-tuning in round r
        _, val_accuracy_rewind = test(model, val_loader, loss_func, True)
        _, test_accuracy_rewind = test(model, test_loader, loss_func)
        
        print('Currrent number of nonzero weights per layer: ')
        num_nonzero_weights_total = 0
        for p_dpu in DPU_layers:
            num_nonzero_weights_total += p_dpu.num_updated_weights
            print(p_dpu.num_updated_weights)
        print('Currrent total number of nonzero weights: ', num_nonzero_weights_total) 


        print('Sparsely fine-tuning...')
        # Conduct the sparse fine-tuning
        # Check if the validation accuracy after pruning is higher than the start point to avoid unnecessary updating
        if val_accuracy_rewind[0] >= val_accuracy_init[0]:
            # If higher, save the current model
            save_model(current_model_path, model, DPU_layers, last_init_round, num_samples)
            round_accuracy[r+1,0] = val_accuracy_rewind[0]
            round_accuracy[r+1,1] = test_accuracy_rewind[0]
            best_acc = val_accuracy_rewind[0]
        else:
            # If lower, use the start point, i.e., may save the communication by not updating
